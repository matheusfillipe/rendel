...
describe('soundfonts', () => {
  ...
  const instruments = [
    'piano', 'flute', 'violin', 'trumpet', 'acoustic_bass',
    'cello', 'church_organ', 'acoustic_guitar_nylon',
  ];

  for (const inst of instruments) {
    it(`s("${inst}") produces non-silent audio`, async () => {
  ...
  it('gm_ prefix works directly', async () => {
    const buf = await renderCode('note("c4").s("gm_piano").gain(0.5)', { duration: 2 });
    expect(isSilent(buf)).toBe(false);
  }, T);

  it('short alias matches gm_ prefix', async () => {
  ...
  it('soundfont with effects chain works', async () => {
