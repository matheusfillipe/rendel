...
import { getPeak, isSilent } from './helpers/audio.js';
...
describe('synths', () => {
  ...
  for (const synth of synths) {
    it(
      `${synth} produces non-silent audio`,
      async () => {
  ...
  it(
    'supersaw produces non-silent audio',
    async () => {
...
describe('samples', () => {
  ...
  for (const name of sampleNames) {
    it(
      `s("${name}") produces non-silent audio`,
      async () => {
  ...
  it(
    'sample begin/end slicing works',
    async () => {
...
describe('effects', () => {
  ...
  for (const { name, code } of effects) {
    it(
      `effect ${name} doesn't crash and produces audio`,
      async () => {
  ...
  it(
    'chained effects work',
    async () => {
      const buf = await renderCode('s("bd").room(0.5).delay(0.3).lpf(2000).gain(0.8)', {
        duration: 4,
      });
      expect(isSilent(buf)).toBe(false);
    },
...
describe('pattern operations', () => {
  ...
  it(
    'struct() applies rhythmic structure',
    async () => {
  ...
  it(
    'jux() applies effect to one channel',
    async () => {
  ...
  it(
    'sometimes() randomly applies modification',
    async () => {
  ...
  it(
    ...
    async () => {
      const buf = await renderCode('stack(s("bd").gain(0.5), note("c3").s("sine").gain(0.3))', {
        duration: 2,
      });
      expect(isSilent(buf)).toBe(false);
    },
